...
imagePaths = list(paths.list_images('regression'))
correctPred = 0
totalPred = 0
NewDataPath = "newDataset/regression/"
...
for imagePath in tqdm(imagePaths):
    ...
    totalValue = int(imgName.split("_")[0])
    ...
    # print(len(coins))
    for coin in coins:
        im_rgb = cv2.cvtColor(coin, cv2.COLOR_BGR2RGB)
        img_res = cv2.resize(im_rgb, (96,96))
        norm_img = img_res/255

        np_img = np.expand_dims(norm_img, axis=0)
        # formatedCoin.append(np_img)
        pred = model.predict(np_img)
        coinType = coinsType[int(np.argmax(pred))]
        # print(coinType)
        coinsValues.append(int(coinsType[int(np.argmax(pred))]))
    
    # print(sum(coinsValues))
    # print(totalValue)
    # print(imagePath)
    totalPred +=1
    if(sum(coinsValues) == totalValue):
        correctPred +=1
        for idx, coin in enumerate(coins):
            cv2.imwrite(NewDataPath + str(coinsValues[idx]) + "/" + str(idx) + imgName, coin)           
    # cv2.circle(img, (x, y), r, (255, 0, 0), 3)
...
print(correctPred)
print(totalPred)
print((correctPred/totalPred) * 100)
